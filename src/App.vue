<!-- 
  Made by Frank Ma
  A Vue2 application which allows user to search cities for their information and put markers on the map. 
 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      initMap() { //For some reason google maps API dont work without the comment below
    ...
    this.map.setCenter({lat, lng}); //centre map
    this.markers.push(marker); //add marker
    ...
    try {
      ...
      const timeZoneResponse = await axios.get(`https://maps.googleapis.com/maps/api/timezone/json`, {
        params: {
          location: `${lat},${lng}`,
          timestamp,
          key: 'AIzaSyD552SEy5_TNtKslTtrb60EvJc0ZlksB-s', //google maps api key
        },
      });
      ...
      const localTime = new Date((timestamp + timeZoneData.dstOffset + timeZoneData.rawOffset) * 1000);     // Calculate local time
      ...
      this.locations.push(locationData); //Add information onto the table
      this.latestLocation = locationData;
      this.nextId += 1;
    } catch (error) {
      console.error('An error occurred!', error); // Google Maps API sometimes cause errors, thus need to catch it
      alert('Failed to fetch time zone data');
    }
  },

  updateSelected(value) { //Update the table
    this.selected = value;
  },

  deleteSelected() { //To remove selected rows of data
